...
const StepperForm = ({ steps, submitHandler, navigateLink, persistKey }: IStepsProps) => {
  ...
  const [current, setCurrent] = useState<number>(
    !!getLocalStorage('step') ? Number(JSON.parse(getLocalStorage('step') as string).step) : 0
  );

  const [savedValues, setSavedValues] = useState(
    !!getLocalStorage(persistKey) ? JSON.parse(getLocalStorage(persistKey) as string) : ''
  );
